...
class Trainer(object):
    ...
    def handle_new_epoch(self):
        ...
        if self.train_epoch % self.eval_period == 0 and self.train_epoch != 0:
            ...
            result = evaluate_crowd_no_overlap(self.model, self.val_dl, device)
            t2 = time.time()
    ...
    def save(self, ):
        ...
        if os.path.exists(self.best_models[-1]):
            shutil.copy(self.best_models[-1], os.path.join(self.output_dir, 'best.pth'))
        else:
            print(f"Source file {self.best_models[-1]} does not exist. Please check the path or ensure the file is generated.")
...
def HungarianMatch(p_gt,p_prd,threshold=0.25):
    ...
    for b in p_gt:
        ...
        d_knn = max(d_knn, 1e-6)  # Ensure d_knn is not zero (avoid division by zero)
        for a in p_prd:
...
def pointF1_score(TP,p_gt,p_prd):
    # TP:cost_martix[row_ind, col_ind]距离小于20的个数
    # TN：锚点数-gt-FP (TN+FP=锚点数-gt)
    # FP：len(p_prd)-TP or?(距离大于20的个数+多预测的即max((p_prd-p_gt),0)）
    # FN：len(p_gt)-TP (TP+FN=len(p_gt))
    FP = int(len(p_prd)) - TP
    FN = int(len(p_gt)) - TP
...
def evaluate_crowd_counting(model, data_loader, device, threshold=0.5, vis_dir=None):
    ...
    with torch.no_grad():
        for samples, targets in data_loader:
            ...
            anchornum = 4096
            F1point_score_ori, Precision_score_ori, Recall_score_ori = pointF1_score(TP, targets[0]['point'],points)
            # if specified, save the visualized images
            if vis_dir is not None: 
                vis(samples, targets, [points], vis_dir)
            # accumulate MAE, MSE
            ...
            maes.append(float(mae))
            mses.append(float(mse))
            f1s.append(float(F1point_score_ori))
            precisions.append(float(Precision_score_ori))
            recalls.append(float(Recall_score_ori))
            print("mae:{},f1:{}".format(mae,F1point_score_ori))
    # calc MAE, MSE
    mae = np.mean(maes)
    mse = np.sqrt(np.mean(mses))
    f1 = np.mean(f1s)
    ...
    # return mae, mse
    #print("mae:{},f1:{}".format(mae,f1))
    return mae, mse,f1,precision,recall
